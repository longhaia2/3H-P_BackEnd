...
public interface ResultVocabularyReponsitory extends JpaRepository<ResultVocabulary, Integer> {
    ...
    @Query(value = "select u.username, e.code_exam, e.content,g.score, g.id, g.exam_id,g.date_test, e.total_question from result_vocabulary as g,user as u, exam as e where u.id=g.user_id and e.id=g.exam_id", nativeQuery = true)
