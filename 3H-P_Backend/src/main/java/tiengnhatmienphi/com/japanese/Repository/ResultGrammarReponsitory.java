...
public interface ResultGrammarReponsitory extends JpaRepository<ResultGrammar, Integer> {
    ...
    @Query(value = "select u.username, e.code_exam, e.content,g.score, g.id, g.exam_id from result_grammar as g,user as u, exam as e where u.id=g.user_id and e.id=g.exam_id", nativeQuery = true)
